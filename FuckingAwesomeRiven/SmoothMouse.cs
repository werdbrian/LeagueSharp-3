...
namespace FuckingAwesomeRiven
{
    class SmoothMouse
    {
        ...
        public static void addMouseEvent(Vector3 pos, bool click = false, bool back = false)
        {
            if (lastAdd + 30 < Environment.TickCount)
            {
        ...
        public static void start()
        {
            new Thread(() =>
            {
                while (MenuHandler.Config.Item("streamMouse").GetValue<bool>())
                {
                    if (queuePos.Count > 0)
                    {
